...
def backup_contents(src_folder, dst_folder):
    """Copy the CONTENTS of src_folder into dst_folder flat — not the folder itself."""
    sp, dp = Path(src_folder), Path(dst_folder)
    dp.mkdir(parents=True, exist_ok=True)
...
def find_current_save_folder(source_folder):
    """
    The game creates a new numeric folder each update (e.g. 820064861 -> 820064862).
    This finds whatever numeric folder currently exists in the same parent dir.
    Returns the Path to use for restore, or None if nothing found.
    """
    sp = Path(source_folder)
    parent = sp.parent
    if not parent.exists():
        return None
    numeric_folders = [f for f in parent.iterdir() if f.is_dir() and f.name.isdigit()]
    if not numeric_folders:
        return None
    if sp.exists():
        return sp
    # Original gone — return the newest numeric folder
    numeric_folders.sort(key=lambda f: f.stat().st_mtime, reverse=True)
    return numeric_folders[0]
...
class App(tk.Tk):
    ...
    def _build(self):
        ...
        self._path_row(path_frame, "SOURCE", "Game save folder (the numbered one)", "source")
        ...
        # ── Info box ──
        info = tk.Frame(self, bg="#0d0800",
                        highlightbackground="#2a1800", highlightthickness=1)
        info.pack(fill="x", padx=20, pady=(4, 10))
        tk.Label(info,
                 text="💡  Set SOURCE to the numbered folder (e.g. ...\\save\\820064861). "
                      "After an update creates a new number, Restore will find it automatically.",
                 font=("Segoe UI", 7), bg="#0d0800", fg="#886633",
                 wraplength=480, justify="left").pack(padx=10, pady=7)
        ...
        tc_frame.pack(fill="x", padx=20, pady=(8, 4))
        ...
        tk.Label(sb, text="created with 💗 by sxnjxxth",
                 ...
                 bg="#050505", fg="#5a2a2a").pack(side="right", padx=14)
    ...
    def _sep(self, margin=0, top=0):
        tk.Frame(self, bg=self.BORDER, height=1).pack(fill="x", padx=margin, pady=(top, 0))

    def _path_row(self, parent, tag, hint, key):
        lrow = tk.Frame(parent, bg=self.BG); lrow.pack(fill="x", pady=(6, 2))
        tk.Label(lrow, text=tag, font=self.LABEL_FONT,
                 bg=self.BG, fg=self.RED_TEXT).pack(side="left")
        tk.Label(lrow, text=f"  u2014  {hint}", font=("Segoe UI", 7),
                 bg=self.BG, fg=self.FG_DIM).pack(side="left")
        row = tk.Frame(parent, bg=self.BG); row.pack(fill="x", pady=(0, 4))
        ...
        def pick_file(k=key, v=var):
            p = filedialog.askopenfilename(title=f"Pick save file")
            if p: v.set(p); self.cfg[k] = p; save_config(self.cfg)
        def pick_folder(k=key, v=var):
            p = filedialog.askdirectory(title=f"Pick save folder")
            if p: v.set(p); self.cfg[k] = p; save_config(self.cfg)
    ...
    def _action_card(self, parent, icon, tag, desc, btn_text, btn_bg, btn_hover, command, side):
        ...
        tk.Button(inner, text=btn_text, font=("Consolas", 9, "bold"),
                  bg=btn_bg, fg="white", relief="flat",
                  activebackground=btn_hover, activeforeground="white",
                  bd=0, cursor="hand2", pady=10, command=command).pack(fill="x", pady=(6, 0))
    ...
    def do_save(self):
        ...
        dst = self.cfg["dest"].strip()
        sp = Path(src)
        if not sp.exists():
            messagebox.showerror("Not Found", f"Source not found:\n{src}"); return
        try:
            if sp.is_dir():
                backup_contents(src, dst)
                msg = f"Contents of '{sp.name}' backed up"
            else:
                copy_path(src, dst)
                msg = f"'{sp.name}' backed up"
            ts = datetime.now().strftime("%H:%M:%S")
            self._status(msg, self.RED_TEXT)
    ...
    def do_restore(self):
        ...
        # Find the current save folder (might be a new number after update)
        target_folder = find_current_save_folder(src)

        if target_folder is None:
            messagebox.showerror("Folder Not Found",
                f"Could not find any save folder in:\n{Path(src).parent}\n\n"
                "Make sure the game has been launched at least once after the update."); return

        # Tell user if folder changed
        sp = Path(src)
        if target_folder.name != sp.name:
            msg = (f"Original folder '{sp.name}' is gone.\n"
                   f"Found new folder: '{target_folder.name}'\n\n"
                   f"Restore backup files into '{target_folder.name}'?")
            if not messagebox.askyesno("New Folder Detected", msg):
                return

        try:
            # Paste backup contents INTO the target folder
            backup_contents(str(dp), str(target_folder))
            ts = datetime.now().strftime("%H:%M:%S")
            self._status(f"Restored into '{target_folder.name}'", self.GOLD_HOT)
